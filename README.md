...
> **About these screenshots.** They are generated by
> [`scripts/render_readme_screenshot.py`](./scripts/render_readme_screenshot.py),
> which renders the real window offscreen against in-memory collaborators so the
> images are reproducible and never touch a developer's own clips folder. The
> sample recordings are genuinely encoded by FFmpeg and thumbnailed through the
> same code path the library uses; their content is a generated gradient rather
> than real gameplay footage.
